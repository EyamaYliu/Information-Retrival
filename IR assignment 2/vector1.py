...
#    Using Singular Value Decomposation to decompose 
#the term vector into the three sub-matrices
#    It is optional to output the original matrix and 
#three demension reduced to file. Just uncomment below

def svdDecompose(Vector):
    Matrix=[]
    terms={}
    cnt=0

    print("Feeding vector to matrix")

    #find all words in the corpus
    for dv in Vector:
        for term in dv.keys():
    ...
    #np.savetxt('OriginalMatrix.txt',Matrix,delimiter = '')

    print("Starting Singular Value Decomposition")
    U, S, V = np.linalg.svd(Matrix,full_matrices = False)
    print("Finding where sv insiginificant")
    ...
    print("Singular Value are insignificant at",emptyMark,". Reducing dimension")
    Ud = U[:,:emptyMark]
    Sd = S[:emptyMark]
    Vd = V[:emptyMark,:]

    #np.savetxt('Ud.txt',Ud,delimiter = '')
    #np.savetxt('Sd.txt',Sd,delimiter = '')
    #np.savetxt('Vd.txt',Vd,delimiter = '')

    return Ud,Sd,Vd

####Reconstruct the three dimension reduced matrices 
#from SVD decomposition to Matrix. Also it is optional
#to output the reconstructed matrix to file
def svdReconstruct(U,S,V):
    d = S.size
    Sd = np.zeros((d, d), int)
    for i in range (0,d):
        Sd[i][i] = S[i]
    ...
    matrix = (U.dot(Sd)).dot(V)

   # np.savetxt('Reconstructed.txt',matrix,delimiter = '')

    return matrix
    


##Calling the SVD functions

#u,s,vh = svdDecompose(doc_vector)
#matrix = svdReconstruct(u,s,vh)
...
def comp_recall(relevance_qry_hash, vect_num):
    ...
    for num in res_vector:
        if relevance_qry_hash[num]:
            similarDoc.append(num)

    simDocLength = len(similarDoc)
    ...
    while retrieved < simDocLength:
        docNum = res_vector[num-1]
        if docNum in similarDoc:
            retrieved += 1
        precArrays.append(float(retrieved/num))
        num += 1

    prec25 = precArrays[int(simDocLength*0.25)]
    prec50 = precArrays[int(simDocLength*0.5)]
    prec75 = precArrays[int(simDocLength*0.75)]
    prec100 = precArrays[int(-1)] #Prec25/50/75/100 calculated

    precMean1 = (prec25 + prec50 + prec75)/3 #PrecMean1 calculated

    Mean2idx = np.arange(0,simDocLength,simDocLength/10)
    Mean2idx = [int(idx) for idx in Mean2idx]
    precMean2 = 0
    for idx in Mean2idx:
        precMean2 += precArrays[idx]

    precMean2 /= 10 #PrecMean2 calculated
    ...
    for rank in range(len(res_vector)):
        if res_vector[rank] in similarDoc:
            rankSum += rank + 1 - ki
            logrankSum += math.log(rank+1) - math.log(ki)
            ki += 1


    normRecall = 1-(rankSum/(simDocLength*(N - simDocLength)))
    normPrec = 1 - logrankSum / (N * math.log(N)-(N - simDocLength)*math.log(N - simDocLength)-simDocLength*math.log(simDocLength))

    result = [prec25,prec50,prec75,prec100,precMean1,precMean2,normRecall,normPrec]
    return result
...
def show_relvnt(relevance_qry_hash, vect_num, my_qry_vector):
    menu = " ************************************************************\n"\
           "                Documents Most Similar To Input               \n"\
           " ************************************************************\n"\
           " Similarity   Doc#  Author      Title                        \n"\
           " ==========   ===  ========     =============================\n"
    sys.stderr.write(menu)
    for each_num in similarDoc:
        similarity = doc_simula[each_num]
        title = titles_vector[each_num][:47]
        print("     " + str(round(similarity,3)) +"   "+title+"\n")
...
def full_cosine_similarity(my_qry_vector, my_doc_vector, num_one, num_two):
    ...
    wfile = open('noEmptyWtlog','a')
    wfile.write('Non-empty terms and weights for query '+str(num_one)+' and document '+str(num_two)+'\n')

    for term in vec1.keys():  
        terms = vec1.get(term, 0) * vec2.get(term, 0)
        if terms != 0:
            print("The word: '",term,"' is not empty")
            print("Weights are: ",str(vec1.get(term, 0))," and ",str(vec2.get(term, 0)))
            wfile.write(str(term)+" "+str(vec1.get(term, 0))+" "+str(vec2.get(term, 0))+"\n")
        cross_product += terms
    result = cross_product / math.sqrt(vec1_norm * vec2_norm)
    ...
    info = "============================================================\n"\
           "{0:11s}  {1:8d}   {2:8d}\n"\
           "============================================================\n".format(
            "Similarity between ", num_one, num_two)
    sys.stderr.write(info)
...
def full_precision_recall_test():
     ...
     for ind in range(1, total_qrys + 1):
         ...
         result = comp_recall( relevance_hash[ ind ], ind)
         print("Just to let you know there are "+str(ind)+" precision and recall function are successfully processed.\n")
...
def dice_sim(vec1,vec2,vec1_norm = 0.0,vec2_norm = 0.0):
    ...
    crossProduct = 2*sum(vec1.get(term, 0) * vec2.get(term, 0) for term in vec1.keys())
    result = crossProduct / (vec1_norm + vec2_norm)
    return result
